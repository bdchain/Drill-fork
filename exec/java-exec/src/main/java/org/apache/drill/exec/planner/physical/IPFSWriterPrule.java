...
public class IPFSWriterPrule extends Prule{
  public static final RelOptRule INSTANCE = new IPFSWriterPrule();

  public IPFSWriterPrule() {
    super(RelOptHelper.some(DrillIPFSWriterRel.class, DrillRel.DRILL_LOGICAL, RelOptHelper.any(RelNode.class)),
        "Prel.IPFSWriterPrule");
  }

  @Override
  public void onMatch(RelOptRuleCall call) {
    final DrillIPFSWriterRel writer = call.rel(0);
    ...
    if (!new WriteTraitPull(call).go(writer, convertedInput)) {
      DrillWriterRelBase newWriter = new IPFSWriterPrel(writer.getCluster(), convertedInput.getTraitSet(),
  ...
  private class WriteTraitPull extends SubsetTransformer<DrillWriterRelBase, RuntimeException> {
    ...
    public RelNode convertChild(DrillWriterRelBase writer, RelNode rel) throws RuntimeException {
      ...
      return new IPFSWriterPrel(writer.getCluster(),
