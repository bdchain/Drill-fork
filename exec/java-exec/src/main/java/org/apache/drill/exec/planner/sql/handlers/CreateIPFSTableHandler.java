...
import org.apache.drill.exec.planner.logical.DrillIPFSWriterRel;
...
public class CreateIPFSTableHandler extends DefaultSqlHandler {
  private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(CreateIPFSTableHandler.class);

  public CreateIPFSTableHandler(SqlHandlerConfig config, Pointer<String> textPlan) {
    super(config, textPlan);
  }

  @Override
  public PhysicalPlan getPlan(SqlNode sqlNode) throws ValidationException, RelConversionException, IOException, ForemanSetupException {
    final SqlCreateIPFSTable sqlCreateIPFSTable = unwrap(sqlNode, SqlCreateIPFSTable.class);
    final String originalTableName = sqlCreateIPFSTable.getName();

    final ConvertedRelNode convertedRelNode = validateAndConvert(sqlCreateIPFSTable.getQuery());
    final RelDataType validatedRowType = convertedRelNode.getValidatedRowType();
    final RelNode queryRelNode = convertedRelNode.getConvertedNode();

    final RelNode newTblRelNode =
        SqlHandlerUtil.resolveNewTableRel(false, sqlCreateIPFSTable.getFieldNames(), validatedRowType, queryRelNode);

    final DrillConfig drillConfig = context.getConfig();
    final AbstractSchema drillSchema = resolveSchema(sqlCreateIPFSTable, config.getConverter().getDefaultSchema(), drillConfig);
    final boolean checkTableNonExistence = sqlCreateIPFSTable.checkTableNonExistence();
    ...
    final RelNode newTblRelNodeWithPCol = SqlHandlerUtil.qualifyPartitionCol(newTblRelNode,
        sqlCreateIPFSTable.getPartitionColumns());

    log("Calcite", newTblRelNodeWithPCol, logger, null);
    // Convert the query to Drill Logical plan and insert a writer operator on top.
    StorageStrategy storageStrategy = sqlCreateIPFSTable.isTemporary() ?
    ...
    // Original table name is stored in temporary tables cache, so it can be substituted to generated one during query  ing.
    String newTableName = sqlCreateIPFSTable.isTemporary() ?
    ...
    DrillRel drel = convertToDrel(newTblRelNodeWithPCol, drillSchema, newTableName,
        sqlCreateIPFSTable.getPartitionColumns(), newTblRelNode.getRowType(), storageStrategy);
    log("Drill Logical##", drel, logger, null);
    Prel prel = convertToPrel(drel, newTblRelNode.getRowType(), sqlCreateIPFSTable.getPartitionColumns());
    ...
    String message = String.format("Creating %s table [%s].",
        sqlCreateIPFSTable.isTemporary()  ? "temporary" : "persistent", originalTableName);
    logger.info(message);
    return plan;
  }
  ...
  private DrillRel convertToDrel(RelNode relNode,
    ...
    final DrillIPFSWriterRel writerRel = new DrillIPFSWriterRel(convertedRelNode.getCluster(),
  ...
  private Prel convertToPrel(RelNode drel, RelDataType inputRowType, List<String> partitionColumns)
    ...
    logAndSetTextPlan("Drill Physical before visitor", prel, logger);
   ...
   *
   * @param sqlCreateIPFSTable create table call
   * @param defaultSchema default schema
   * @param config drill config
   * @return resolved schema
   * @throws UserException if attempted to create temporary table outside of temporary workspace
   */
  private AbstractSchema resolveSchema(SqlCreateIPFSTable sqlCreateIPFSTable, SchemaPlus defaultSchema, DrillConfig config) {
    AbstractSchema resolvedSchema;
    if (sqlCreateIPFSTable.isTemporary() && sqlCreateIPFSTable.getSchemaPath().size() == 0) {
      ...
      resolvedSchema = SchemaUtilites.resolveToMutableDrillSchema(
          defaultSchema, sqlCreateIPFSTable.getSchemaPath());
    }

    if (sqlCreateIPFSTable.isTemporary()) {
